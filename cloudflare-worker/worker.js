 ...
 * and returns them to the bot server.
 * 
 * Endpoints:
...
function decipherSignature(js, sig) {
  ...
  try {
    ...
    const funcBodyMatch = js.match(/([a-zA-Z0-9$]+)\s*=\s*function\(\s*a\s*\)\s*\{\s*a\s*=\s*a\.split\(\s*""\s*\)\s*;\s*([\s\S]*?)\s*return\s+a\.join\(\s*""\s*\)/);
    if (!funcBodyMatch) return sig;
    
    const funcBody = funcBodyMatch[0];
    const objectName = funcBody[1];
    
    // Extract the helper object
    const helperMatch = js.match(new RegExp(`var\\s+${objectName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*=\\s*\\{([\\s\\S]*?)\\}\\s*;`));
    if (!helperMatch) return sig;
    
    const helperBody = helperMatch[0];
    
    // Evaluate the decipher function
    const evalCode = `${helperBody}\n${funcBody}\n;var result = ${objectName}("${sig.replace(/"/g, '\\"')}");`;
    
    // Use a simple VM for evaluation
    const result = evalInContext(evalCode);
    return result || sig;
  } catch (e) {
    return sig;
  }
...
function evalInContext(code) {
  // Cloudflare Workers support eval() in some contexts
  // But we'll use a safer approach with Function constructor
  try {
