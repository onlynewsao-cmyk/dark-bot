...
async function bootstrap() {
  ...
  // Trust proxy (necessário para Render / HTTPS)
  ...
  const sessionConfig = {
    ...
    cookie: {
      ...
      secure: config.nodeEnv === 'production', // true apenas em produção (HTTPS)
      sameSite: 'lax', // Mais compatível que 'none' para o mesmo domínio
      httpOnly: true,
    },
  };
  if (conn) {
    try {
      // Usa a conexão EXISTENTE do mongoose — evita criar segunda conexão que falha no Render
      const client = conn.getClient();
      sessionConfig.store = MongoStore.create({
        client,
        collectionName: 'web_sessions', // Evita conflito com sessões do Baileys
        ttl: 60 * 60 * 24 * 7,
        touchAfter: 24 * 3600,
        autoRemove: 'native',
      });
      console.log('💾 MongoStore ativado (via conexão existente)');
    } catch (storeErr) {
      console.warn('⚠️ MongoStore falhou:', storeErr.message, '— sessão será em memória (perde login após restart)');
    }
  } else {
    console.warn('⚠️ MongoDB offline — sessão em memória ativada (login não persiste entre restarts)');
  }
