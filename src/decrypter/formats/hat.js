...
async function parse(buffer, fileName) {
  ...
  try { data = JSON.parse(json); }
    ...
    return { configName: fileName.replace(/\.hat$/i, ''), configType: 'HA Tunnel Plus', note: 'JSON malformado', raw: json };
  }
...
function parseHatJson(data, fileName) {
  ...
  return {
    configName: find('configName', 'name', 'config_name', 'profileName') || fileName.replace(/\.hat$/i, ''),
    configType: 'HA Tunnel Plus',
    appName: 'HA Tunnel Plus',
    ...
    host: find('proxyHost', 'host', 'sshHost', 'server', 'proxy_host'),
    port: find('proxyPort', 'port', 'sshPort', 'server_port', 'proxy_port'),

    ssh: {
      host: find('sshHost', 'ssh_host', 'sshServer'),
      port: find('sshPort', 'ssh_port', 'sshServerPort'),
      user: find('sshUsername', 'username', 'sshUser', 'ssh_user', 'ssh_username'),
      pass: find('sshPassword', 'password', 'sshPass', 'ssh_pass', 'ssh_password'),
    },
