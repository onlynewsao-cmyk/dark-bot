...
async function parse(buffer, fileName) {
  ...
  if (!json) {
    ...
    const textFields = forensic.fields;
    ...
    const note = binHeader
      ? `🔒 *HTTP Injector v${binHeader.appVersion} — formato binário protegido*\n\n` +
        ...
        `├ Conteúdo interno não expôs JSON legível\n` +
        `└ Proteção compatível com configs EHI recentes\n\n` +
        `⚠️ Resultado classificado como *parcial/protegido*.`
...
function parseEhiJson(data, fileName) {
  ...
  const sshPass = find('ssh_password', 'password', 'ssh_pass', 'sshPassword', 'sshPass');
    ...
    return {
      ...
      note: find('config_note', 'note', 'notes', 'description', 'config_description', 'config_note', 'message'),

      host: find('proxy_ip', 'proxy_host', 'host', 'remote_proxy', 'proxyHost', 'real_proxy_host', 'ssh_host', 'server'),
      port: find('proxy_port', 'port', 'proxyPort', 'real_proxy_port', 'ssh_port', 'server_port'),
      connectionType: find('connection_mode', 'mode', 'connectionType'),

      ssh: {
        host: find('ssh_host', 'sshHost', 'ssh_server', 'server'),
        port: find('ssh_port', 'sshPort', 'ssh_server_port', 'server_port'),
        user: sshUser,
        pass: sshPass,
      },
