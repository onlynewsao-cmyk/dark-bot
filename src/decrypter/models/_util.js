...
function extractFieldsFromText(text) {
  const fields = {};
  const patterns = {
    host: /(?:host|server|proxy[_\s]?host|remote[_\s]?host|ip|addr(?:ess)?|address)\s*[:=]\s*([a-zA-Z0-9._-]+\.[a-z]{2,})/i,
    port: /(?:port|proxy[_\s]?port|server[_\s]?port|remote[_\s]?port)\s*[:=]\s*(\d{1,5})/i,
    sshHost: /(?:ssh[_\s]?host|ssh[_\s]?server)\s*[:=]\s*([a-zA-Z0-9._-]+\.[a-z]{2,})/i,
    sshPort: /(?:ssh[_\s]?port|ssh[_\s]?server_port)\s*[:=]\s*(\d{1,5})/i,
    sshUser: /(?:ssh[_\s]?user(?:name)?|user(?:name)?|login|ssh_username)\s*[:=]\s*([^\s,;}\]"']+)/i,
    sshPass: /(?:ssh[_\s]?pass(?:word)?|pass(?:word)?|pwd|ssh_password)\s*[:=]\s*([^\s,;}\]"']+)/i,
    sni: /(?:sni|sni[_\s]?host|ssl[_\s]?sni|tls[_\s]?sni)\s*[:=]\s*([a-zA-Z0-9._-]+\.[a-z]{2,})/i,
    payload: /(?:payload|custom[_\s]?payload|request[_\s]?payload)\s*[:=]\s*([^\n]+)/i,
    dns: /(?:dns|dns[_\s]?server)\s*[:=]\s*([0-9.]+)/i,
    proxyHost: /(?:proxy[_\s]?host|proxy[_\s]?ip)\s*[:=]\s*([a-zA-Z0-9._-]+\.[a-z]{2,})/i,
  ...
  // Busca por hosts genéricos (domínios e IPs)
  const hostRegex = /(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]/gi;
  const allHosts = text.match(hostRegex) || [];
  const filtered = allHosts.filter(h => 
    !h.toLowerCase().includes('evozi') && 
    !h.toLowerCase().includes('google') && 
    !h.toLowerCase().includes('injector') &&
    !/^\d+(\.\d+)+$/.test(h) // Ignora versão se for só números
  );
  
  if (!fields.host && filtered.length > 0) fields.host = filtered[0];
