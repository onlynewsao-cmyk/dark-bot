...
function detectFormat(fileName, buffer) {
  ...
  // Prioriza detecção por conteúdo para URIs VPN (antes da extensão)
  const textCheck = buffer.toString('utf-8').trim().toLowerCase();
  if (textCheck.startsWith('bdnet://')) return 'bdnet';
  if (textCheck.startsWith('apnalite://') || textCheck.startsWith('apna://')) return 'apnalite';
  if (textCheck.startsWith('wyrvpn://')) return 'wyrvpn';
...
/**
 * Detecta e faz download de cloud links embutidos nos ficheiros.
 * HTTP Injector Cloud, NPV Cloud, etc. guardam uma URL dentro do ficheiro.
 * Suporta: ehCloud, npvCloud, cloudUrl, cloud_url, remoteConfig, etc.
 */
async function tryCloudDownload(buffer) {
  const text = buffer.toString('utf-8').slice(0, 2000);

  // Padrões de URL cloud em vários formatos
  const urlPatterns = [
    /["']?(?:cloud_?url|cloudUrl|ehCloud|npvCloud|remoteConfig|remote_url|config_url|update_url|server_url)["']?\s*[:=]\s*["']?(https?:\/\/[^\s"',}\]>]+)/i,
    /https?:\/\/[a-zA-Z0-9.-]+\.[a-z]{2,}\/[^\s"'<>]{10,}\.(?:ehi|ehic|hat|npv4|npv7|npv8|npvt|dark|any|tls|nm|nmess|conf|ovpn|ssh|json)[^\s"'<>]*/i,
  ];
  ...
  for (const re of urlPatterns) {
    ...
    if (m) {
      ...
      try {
        ...
        const cloudBuf = await Promise.race([
          mediaHandler.fetchBuffer(url),
          new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 8000)),
        ]);
        if (cloudBuf && cloudBuf.length > 10) {
          return { buffer: cloudBuf, url };
        }
      } catch (e) { /* ignora — URL inacessível */ }
    }
  }
  return null;
}
...
async function decrypt(fileName, buffer) {
  // Tenta cloud download primeiro (se o ficheiro contiver uma URL cloud)
  try {
    const cloud = await tryCloudDownload(buffer);
    if (cloud) {
      const cloudFormat = detectFormat(cloud.url.split('/').pop() || fileName, cloud.buffer);
      if (cloudFormat) {
        const cloudResult = await parsers[cloudFormat].parse(cloud.buffer, fileName);
        if (cloudResult && (cloudResult.host || cloudResult.ssh?.host || cloudResult.vmess || cloudResult.sni || cloudResult.partial)) {
          cloudResult._cloudUrl = cloud.url;
          cloudResult.note = (cloudResult.note || '') + ` (Cloud: ${cloud.url.slice(0, 60)})`;
          const normalizedCloud = normalizeOutput(cloudResult);
          return {
            success: !normalizedCloud.partial,
            partial: !!normalizedCloud.partial,
            format: cloudFormat.toUpperCase() + ' (Cloud)',
