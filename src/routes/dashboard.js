...
router.get('/commands', requireOwner, async (req, res) => {
  const commands = await Command.find().sort({ category: 1, name: 1 }).catch(() => []);
...
router.get('/media', requireOwner, async (req, res) => {
  const medias = await Media.find().sort({ createdAt: -1 }).catch(() => []);
  res.render('dashboard/media', { title: 'Mídias', medias });
});
router.get('/users', requireOwner, async (req, res) => {
  const users = await User.find().sort({ createdAt: -1 }).catch(() => []);
  res.render('dashboard/users', { title: 'Usuários', users });
});

router.get('/payments', requireOwner, async (req, res) => {
  const payments = await Payment.find().sort({ createdAt: -1 }).limit(200).catch(() => []);
  const all = await Payment.find().catch(() => []);
  const stats = {
...
router.get('/internet', requireOwner, async (req, res) => {
  ...
  let configs = [];
  try {
    updates = await BotConfig.get('internet_updates', []);
    configs = await BotConfig.get('clipboard_configs', []);
  } catch (e) {
    console.warn('[Dashboard] BotConfig falhou:', e.message);
  }
  const all = await BotConfig.find().catch(() => []);
  const settings = {};
  all.forEach(c => { settings[c.key] = c.value; });
