...
module.exports = function (io) {
  ...
  router.post('/groups/join', requireApiAuth, async (req, res) => {
    try {
      const { link } = req.body;
      const bot = getBot(io);
      if (bot.getStatus().status !== 'connected') return res.status(400).json({ error: 'Bot offline' });
      
      const code = link.split('chat.whatsapp.com/')[1];
      if (!code) return res.status(400).json({ error: 'Link inválido' });
      
      const jid = await bot.sock.groupAcceptInvite(code);
  ...
  router.post('/bot/start', requireApiOwner, async (req, res) => {
    const { mode, phoneNumber } = req.body;
    const bot = getBot(io);
    try {
      bot.start({ mode: mode || 'qr', phoneNumber: phoneNumber || null })
