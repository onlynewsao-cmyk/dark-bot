...
const GROUP_META_TTL = 60000; // 1 minuto
...
async function handle(sock, msg) {
  ...
  // Verifica se é dono — por número OU por LID
  const ownerLid = await botConfigCache.get('owner_lid', '');
  ...
  // Blacklist — CACHE em memória
  const blacklist = await botConfigCache.get('blacklist', []);
  ...
  // Detecta URI VPN em texto puro (bdnet://, apnalite://, vmess://, etc)
  const vpnUriMatch = text.match(/^(bdnet|apnalite|apna|vmess|vless|trojan|ss|ssr|ssh|hysteria|hysteria2|tuic|warp):\/\/[^\s]{10,}$/i);
  if (vpnUriMatch && !text.startsWith(prefix)) {
    const user = await require('../database/models/User').findOne({ whatsappNumber: ctx.senderNumber });
    const isPremium = isOwner || (user && user.isPremium());
    if (isPremium) {
      await sock.sendMessage(ctx.remoteJid, { react: { text: '🔓', key: msg.key } });
      try {
        // Usa extensão do scheme para detecção correta
        const scheme = vpnUriMatch[1].toLowerCase();
        const fakeFileName = `config.${scheme === 'apna' ? 'apnalite' : scheme}`;
        const result = await decrypter.decrypt(fakeFileName, Buffer.from(text.trim()));
  ...
  // Detecta hex puro longo (BD Net/APNA sem URI prefix — ex: 4f0736...)
  if (/^[0-9a-fA-F]{100,}$/.test(text.trim()) && text.trim().startsWith('4f07') && !text.startsWith(prefix)) {
    const user = await require('../database/models/User').findOne({ whatsappNumber: ctx.senderNumber });
    const isPremium = isOwner || (user && user.isPremium());
    if (isPremium) {
      await sock.sendMessage(ctx.remoteJid, { react: { text: '🔓', key: msg.key } });
      try {
        const result = await decrypter.decrypt('config.bdnet', Buffer.from(text.trim(), 'hex'));
        const formatted = formatForWhatsApp(result, config);
        await sock.sendMessage(ctx.remoteJid, { text: formatted }, { quoted: msg });
        await sock.sendMessage(ctx.remoteJid, { react: { text: '✅', key: msg.key } });
        return true;
  ...
  if (docMsg) {
    ...
    const supportedExts = ['ehi','ehic','hat','npv','npv4','npv7','npv8','npvt','dark','darkt','any','tls','conf','nm','nmess','ovpn','ssh','ssl','json','txt','bdnet','apnalite'];
