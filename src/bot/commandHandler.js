...
async function handle(sock, msg) {
  ...
  if (user && user.gender === 'unknown' && !text.includes('genero')) {
    const isGenderCmd = text.includes('homem') || text.includes('mulher');
    if (!isGenderCmd) {
      const msgGender = `👋 Olá *${ctx.pushName}*! Eu sou o *${config.bot.name}*.\n\nNotei que é sua primeira vez aqui. Como você gostaria de ser tratado?\n\n1️⃣ Digite *!genero homem*\n2️⃣ Digite *!genero mulher*`;
      await sock.sendMessage(ctx.remoteJid, { text: msgGender }, { quoted: msg });
      return true;
    }
  }

  // Se já tem gênero, o bot pode ocasionalmente saudar (opcional)
  if (user && user.gender !== 'unknown' && Math.random() > 0.95) {
      const saudacao = user.gender === 'male' ? 'Bem-vindo de volta, meu Rei! 👑' : 'Bem-vinda de volta, minha Rainha! ✨';
      console.log(`[BOT] Reconhecido: ${ctx.senderNumber} - ${saudacao}`);
  }
