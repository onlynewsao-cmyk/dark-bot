...
async function handle(sock, msg) {
  ...
  // Identifica usuário (cria/atualiza sem duplicar)
  let dbUser = null;
  try {
    dbUser = await userManager.identifyByWhatsApp(ctx.senderNumber, ctx.pushName);
    // Atualiza lastSeenAt
    if (dbUser) {
      dbUser.lastSeenAt = new Date();
      await dbUser.save().catch(() => {});
    }
  } catch (e) { console.error('userManager err:', e.message); }
  ctx.dbUser = dbUser;
  ...
  // ===== DECRYPT por URI direta no texto (bdnet://, apnalite://, wyrvpn://, vmess://, etc) =====
  const uriMatch = text.match(/(bdnet|apnalite|apna|wyrvpn|wyr|vmess|vless|trojan|ss|ssh):\/\/[\w\d+/=:.@%-]+/i);
  if (uriMatch) {
    const scheme = uriMatch[1].toLowerCase();
    const uri = uriMatch[0];
    const isDecRequest = prefixes.some(p =>
      text.toLowerCase().startsWith(`${p}decrypt`) || text.toLowerCase().startsWith(`${p}vpn`) ||
      text.toLowerCase().startsWith(`${p}dec`)
    );

    // Decrypta automaticamente se for URI conhecida no texto
    if (isDecRequest || ['bdnet','apnalite','apna','wyrvpn','wyr'].includes(scheme)) {
      try {
        ...
        const decrypter = require('../decrypter');
        const { formatForWhatsApp } = require('../decrypter/formatter');
        const result = await decrypter.decrypt(scheme + '.txt', Buffer.from(uri));
        const formatted = formatForWhatsApp(result, config);
        if (formatted.length > 4000) {
          for (let i = 0; i < formatted.length; i += 3800) {
            await sock.sendMessage(ctx.remoteJid, { text: formatted.slice(i, i + 3800) }, { quoted: msg });
          }
        } else {
  ...
  if (packageCommands[commandName]) {
    try {
      ...
      await incrementUserCommand(ctx.senderNumber, ctx, commandName);
      return true;
    } catch (err) {
...
async function incrementUserCommand(number, ctx, commandName = '') {
  try {
    const u = ctx?.dbUser || await userManager.identifyByWhatsApp(number);
    if (u) {
    ...
    // Live broadcast
    liveBroadcaster.userCommand({
      command: commandName,
      user: ctx?.pushName || u?.name || 'Unknown',
      number,
      group: ctx?.isGroup ? ctx.groupName : null,
      role: u?.role || 'free',
