...
async function handle(sock, msg) {
  ...
  if (packageCommands[commandName]) {
    try {
      await packageCommands[commandName]({ sock, msg, ctx, args, isOwner, fillVars, config: commandConfig });
      if (groupConfig) {
  ...
  if (nativeCommands[commandName]) {
    try {
      await nativeCommands[commandName]({ sock, msg, ctx, args, isOwner, fillVars, config: commandConfig });
      if (groupConfig) {
        groupConfig.commandsUsedToday++;
        groupConfig.totalCommands++;
        await groupConfig.save();
  ...
  if (aliasMap[commandName] && nativeCommands[aliasMap[commandName]]) {
    try {
      await nativeCommands[aliasMap[commandName]]({ sock, msg, ctx, args, isOwner, fillVars, config: commandConfig });
      await incrementUserCommand(ctx.senderNumber);
      return true;
    } catch (err) {
      await sock.sendMessage(ctx.remoteJid, { text: '❌ ' + err.message }, { quoted: msg });
