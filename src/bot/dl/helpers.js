...
async function cobaltDownload(url, mode = 'auto', quality = '720') {
  const instances = [
    COBALT_API_URL,
    ...(process.env.COBALT_API_URLS || '').split(',').map(x => x.trim()),
    // api.cobalt.tools hoje normalmente exige autenticação/bot protection; fica por último.
  ...
  for (const baseUrl of instances) {
    try {
      ...
      const r = await mediaHandler.fetchJsonPost(baseUrl, body, headers, 45000);
      const u = r?.url || r?.tunnel || r?.picker?.[0]?.url || bestUrlFromApiResponse(r, mode);
      if (u) return { url: u, source: 'Cobalt' };
    } catch (e) {
