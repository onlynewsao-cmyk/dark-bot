...
async function pinterestSearch(query) {
  ...
  return tryApis(apis, r => {
    let arr = (r && r.result) || (r && r.data) || (r && r.results) || [];
    if (!Array.isArray(arr)) arr = [];

    // Pega até 10 fotos aleatórias e de boa qualidade
    const shuffled = arr.sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, 10);

    const results = selected.map(pick => {
      const u = (typeof pick === 'string') 
        ? pick 
        : (pick.image || pick.url || pick.src || pick.download_url || pick.thumbnail || pick.media);
      return u ? { url: u } : null;
    }).filter(Boolean);
