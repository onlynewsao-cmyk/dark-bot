...
async function sendAudioWithCard(sock, msg, ctx, r) {
  ...
  // 🔒 CRÍTICO: baixa o áudio localmente para garantir entrega real do arquivo
  // (URL externa pode falhar, expirar ou ser bloqueada pelo WhatsApp)
  ...
  try {
    ...
    console.log('[sendAudio] buffer download failed, fallback to URL:', e.message);
  }
...
async function sendVideoFromUrl(sock, jid, url, caption, quotedMsg, opts = {}) {
  ...
  try {
    ...
    if (kind === 'mp4') {
      // Buffer é MP4 real → envia direto como vídeo (mais rápido)
    ...
    // NÃO é MP4 puro: envia como DOCUMENTO para garantir que o arquivo chegue intacto
    // WhatsApp Android rejeita WebM/AVI/TS como vídeo direto — assim o usuário baixa o arquivo real
    return sock.sendMessage(jid, {
      document: buf,
      fileName: `${title}.mp4`,
      mimetype: 'video/mp4',
    ...
    // Se não conseguiu baixar buffer, envia como URL direto (último recurso)
    return sock.sendMessage(jid, {
      video: { url },
      caption: caption + `\n\n📥 Qualidade: ${opts.quality || 'HD'}`,
      mimetype: 'video/mp4',
