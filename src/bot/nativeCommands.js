...
async function sendStyledCommandList(sock, msg, ctx, config, { title, subtitle, buttonText = '⚡ Abrir comandos', target = 'menu', items = [] }) {
  ...
  try {
    await buttonHandler.sendListWithImage(sock, ctx.remoteJid, `⌁ ${config.bot.name} 🌑`, `Selecione um comando de ${title}:`, buttonText, await getMenuImage(target), [{ title, rows }], msg);
  } catch (e) {
    try { await buttonHandler.sendButtonsWithImage(sock, ctx.remoteJid, `⌁ ${title}`, 'Dark Side Engine ⚡', await getMenuImage(target), visible.slice(0, 8), msg); } catch {}
  }
}
...
╚══════════════════════╝`;
  ...
  async menubtn({ sock, msg, ctx, config }) {
    ...
    for (let i = 0; i < 2; i++) {
      try {
        await buttonHandler.sendListWithImage(sock, ctx.remoteJid, `⌁ ${config.bot.name} 🌑`, 'Escolha um módulo do Dark Side Engine:', '⚡ Abrir módulos', await getMenuImage('menubtn'), [{ title: 'Dark Side Modules ⚡', rows }], msg);
        logCmd('menubtn', ctx);
        return;
      } catch {}
      try {
        await buttonHandler.sendButtonsWithImage(sock, ctx.remoteJid, title, footer, await getMenuImage('menubtn'), items.map(x => ({ id: x.id, text: x.text })), msg);
        logCmd('menubtn', ctx);
        return;
      } catch {}
    }
  ...
  async menudownload({ sock, msg, ctx, config }) {
    return sendStyledCommandList(sock, msg, ctx, config, {
      ...
      items: [
        ...
        { cmd: 'video2', args: 'nome do vídeo', emoji: '📺', desc: 'vídeo FHD/1080p quando possível' },
...
╰━━━━━━━━━━━━━━━━╯`;
  ...
  async pinterest({ sock, msg, ctx, args, config }) {
    ...
    try {
      ...
      const grid = await createImageGrid(selected, 9);
      const caption = `╭━━━〔 📌 PINTEREST GRID 〕━━━╮\n` +
        `┃ Busca: *${args.join(' ')}*\n` +
        `┃ Imagens: *${selected.length}* organizadas\n` +
        `┃ Entrega: grade única anti-bagunça\n` +
        `╰━━━〔 ᴅᴀʀᴋ sɪᴅᴇ ⚡ 〕━━━╯`;
      await sock.sendMessage(ctx.remoteJid, { image: grid, caption }, { quoted: msg });
      await buttonHandler.sendButtons(sock, ctx.remoteJid, '📌 Pinterest organizado', 'Dark Side Engine ⚡', [
        { id: `${config.bot.prefix}pinterest ${args.join(' ')}`, text: '🔁 Mais uma grade' },
        { id: `${config.bot.prefix}menudownload`, text: '📥 Downloads' },
      ], msg).catch(() => {});
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
