...
async function sendAudioWithCard(sock, msg, ctx, r) {
  ...
  const fileName = r.fileName || `${title.replace(/[/\\?%*:|"<>]/g, '-').slice(0, 60)}.mp3`;
  ...
  // 🔒 CRÍTICO: se o downloader retornou buffer real, usa ele.
  // Se não, tenta baixar da URL externa. Se falhar tudo, envia URL direto.
  let audioBuffer = r.buffer || null;
  if (!audioBuffer && r.url && r.url.startsWith('http')) {
    try {
      audioBuffer = await mediaHandler.fetchBuffer(r.url);
    } catch (e) {
      console.log('[sendAudio] buffer download failed:', e.message);
    }
  }

  const audioPayload = audioBuffer ? { audio: audioBuffer } : { audio: { url: r.url } };
...
async function sendVideoFromUrl(sock, jid, r, caption, quotedMsg, opts = {}) {
  const title = sanitizeFileName(opts.title || r.title || 'video');
  const fileName = `${title}.mp4`;

  // 1) Se o downloader retornou buffer real, usa direto (melhor opção)
  if (r.buffer && r.buffer.length > 1000) {
    const buf = r.buffer;
    ...
    // Não é MP4 puro: envia como DOCUMENTO para garantir entrega
    return sock.sendMessage(jid, {
      document: buf,
      fileName,
      mimetype: 'video/mp4',
      caption: caption + `\n\n📥 Qualidade: ${opts.quality || 'HD'}\n⚠️ Enviado como arquivo para compatibilidade`,
    }, { quoted: quotedMsg });
  }

  // 2) Sem buffer, tenta baixar da URL
  if (r.url && r.url.startsWith('http')) {
    try {
      ...
      return sock.sendMessage(jid, {
        ...
        fileName,
        mimetype: 'video/mp4',
        caption: caption + `\n\n📥 Qualidade: ${opts.quality || 'HD'}\n⚠️ Enviado como arquivo`,
      }, { quoted: quotedMsg });
    } catch (e) {
      console.log('[sendVideo] buffer download failed:', e.message);
    }
  }

  // 3) Último recurso: URL direto
  return sock.sendMessage(jid, {
    video: { url: r.url },
    caption: caption + `\n\n📥 Qualidade: ${opts.quality || 'HD'}`,
    mimetype: 'video/mp4',
...
╰━━━━━━━━━━━━━━━╯`);
  ...
  async video({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sendVideoFromUrl(sock, ctx.remoteJid, r, cap, msg, { title: r.title || 'youtube-video' });
      await react(sock, msg, '✅');
  ...
  async tiktok({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sendVideoFromUrl(sock, ctx.remoteJid, r, cap, msg, { title: r.title || 'tiktok-video' });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async instagram({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      if (r.type === 'video') {
        await sendVideoFromUrl(sock, ctx.remoteJid, r, '📸 *Instagram*', msg, { title: 'instagram-video' });
      } else {
  ...
  async fb({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sendVideoFromUrl(sock, ctx.remoteJid, r, '📘 *Facebook*', msg, { title: 'facebook-video' });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async twitter({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sendVideoFromUrl(sock, ctx.remoteJid, r, '🐦 *X / Twitter*', msg, { title: 'twitter-video' });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async pinterest({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      if (isUrl) {
        ...
        const results = await downloader.pinterestSearch(args.join(' '), 10);
        if (!results || !results.length) throw new Error('Nenhuma imagem encontrada.');
        // Envia mensagem de introdução
        await reply(sock, msg, ctx, `📌 *Pinterest:* ${args.join(' ')}\n_Enviando ${results.length} imagens aleatórias..._`);
        // Envia até 10 imagens, uma por uma, com delay para não floodar
        for (let i = 0; i < results.length; i++) {
          try {
            await sock.sendMessage(ctx.remoteJid, {
              image: { url: results[i].url },
              caption: `📌 ${args.join(' ')} (${i+1}/${results.length})`,
            }, { quoted: msg });
            await new Promise(r => setTimeout(r, 800)); // delay entre imagens
          } catch (imgErr) {
            console.error(`[pinterest] img ${i+1} failed:`, imgErr.message);
          }
        }
      }
      await react(sock, msg, '✅');
  ...
  async mediafire({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      // r.buffer já vem do downloader (já baixou localmente)
      if (r.buffer && r.buffer.length > 1000) {
        await sock.sendMessage(ctx.remoteJid, {
          document: r.buffer, fileName: r.title, mimetype: 'application/octet-stream',
  ...
  async apk({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      // Se o primeiro resultado tiver buffer (APK real baixado), envia como documento
      if (results[0]?.buffer && results[0].buffer.length > 10000) {
        const r = results[0];
        await sock.sendMessage(ctx.remoteJid, {
          document: r.buffer,
          fileName: r.fileName || `${r.name.replace(/[^a-z0-9]/gi, '_')}.apk`,
