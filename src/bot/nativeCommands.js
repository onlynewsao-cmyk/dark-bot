...
async function botIsAdmin(sock, ctx) {
  ...
  try {
    const meta = ctx.groupMeta || (await sock.groupMetadata(ctx.remoteJid));
    // O JID do bot pode ter formato "número:device@s.whatsapp.net" ou "número@s.whatsapp.net"
    // Normalizamos para o número base e comparamos sem o device suffix
    const botNum = (sock.user?.id || '').split(':')[0].split('@')[0];
    const p = meta.participants.find(x => x.id.split(':')[0].split('@')[0] === botNum);
    return p?.admin === 'admin' || p?.admin === 'superadmin';
  } catch { return false; }
}
  ...
  async play({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sock.sendMessage(ctx.remoteJid, {
        ...
        contextInfo: { externalAdReply: { title: r.title, body: '🎵 YouTube Audio', thumbnail: Buffer.alloc(0) } },
      }, { quoted: msg });
      await react(sock, msg, '✅');
      logCmd('play', ctx);
    } catch (e) {
