...
/**
 * Baixa vídeo e re-encoda para MP4 H.264 + AAC compatível com WhatsApp.
 *
 * O WhatsApp exige:
 *  - Codec vídeo: H.264 (libx264), perfil baseline/main, nível ≤ 4.0
 *  - Codec áudio: AAC (aac) ou sem áudio
 *  - Container: MP4 (moov atom no início — faststart)
 *  - Resolução: múltiplo de 2 (libx264 exige)
 *
 * Sem re-encode: vídeos com codec VP9, HEVC, AV1 etc. não reproduzem no WhatsApp.
 */
...
function hasFfmpegNative() {
  try { require('child_process').execSync('ffmpeg -version', { stdio: 'ignore' }); return true; } catch { return false; }
}
const _FFMPEG = hasFfmpegNative();

async function fetchVideoBuffer(url, maxMB = 50) {
  const buf = await mediaHandler.fetchBuffer(url);
  if (buf.length > maxMB * 1024 * 1024) {
    throw new Error(`Vídeo muito grande (${(buf.length/1024/1024).toFixed(1)}MB). Máx: ${maxMB}MB`);
  }
  // Re-encoda para H.264 + AAC se ffmpeg disponível
  if (_FFMPEG) {
    return reencodeToMp4(buf);
  }
  return buf;
}

/**
 * Re-encoda buffer de vídeo para MP4 H.264 compatível com WhatsApp.
 * Leve e rápido: usa preset fast, crf 28, resolução máxima 720p.
 */
function reencodeToMp4(inputBuf) {
  const tmp     = _path.join(_os.tmpdir(), `vid_${Date.now()}_${Math.random().toString(36).slice(2)}`);
  const inFile  = `${tmp}.in`;
  const outFile = `${tmp}.mp4`;
  try {
    _fs.writeFileSync(inFile, inputBuf);
    _execFileSync('ffmpeg', [
      '-y',
      '-i', inFile,
      // Vídeo: H.264, perfil main, nível 4.0, CRF 28 (boa qualidade, arquivo pequeno)
      '-c:v', 'libx264',
      '-profile:v', 'main',
      '-level', '4.0',
      '-preset', 'fast',
      '-crf', '28',
      // Resolução: máximo 720p, sempre múltiplo de 2
      '-vf', 'scale=trunc(iw*min(1\\,min(1280/iw\\,720/ih))/2)*2:trunc(ih*min(1\\,min(1280/iw\\,720/ih))/2)*2',
      // Áudio: AAC 128k (se existir áudio) — -c:a copy é inseguro pois pode ter codec incompatível
      '-c:a', 'aac',
      '-b:a', '128k',
    ...
    const result = _fs.readFileSync(outFile);
    if (!result || result.length < 500) throw new Error('Output vazio');
    return result;
  } catch (e) {
    // Se re-encode falhar, retorna buffer original
    return inputBuf;
  } finally {
    try { _fs.unlinkSync(inFile);  } catch {}
    try { _fs.unlinkSync(outFile); } catch {}
  }
}
...
╰━━━━━━━━━━━━━━━╯`);
  ...
  async play({ sock, msg, ctx, args }) {
    if (!args.length) return reply(sock, msg, ctx, '🎵 Use: !play <nome da música>\n\n💡 _Busca no YouTube e baixa o áudio_');
    await react(sock, msg, '⏳');
    try {
      ...
      await react(sock, msg, '❌');
      return reply(sock, msg, ctx, `❌ *Download falhou*\n\n${e.message}\n\n💡 _Tente !play2 <link> ou !soundcloud <nome>_`);
    }
  },

  async play2({ sock, msg, ctx, args }) {
    if (!args.length) return reply(sock, msg, ctx, '🎵 Use: !play2 <nome ou link>\n\n💡 _Download via SaveFrom_');
    await react(sock, msg, '⏳');
    try {
  ...
  async play3({ sock, msg, ctx, args }) {
    if (!args.length) return reply(sock, msg, ctx, '🎵 Use: !play3 <nome da música>\n\n💡 _Busca em YouTube + SoundCloud automaticamente_');
    await react(sock, msg, '⏳');
    try {
  ...
  async video({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      const buf = await fetchVideoBuffer(r.url);
      await sock.sendMessage(ctx.remoteJid, { video: buf, caption: `🎬 *${r.title}*`, mimetype: 'video/mp4' }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },

  async video2({ sock, msg, ctx, args }) {
    if (!args.length) return reply(sock, msg, ctx, '🎬 Use: !video2 <nome ou link>');
    await react(sock, msg, '⏳');
    try {
  ...
  async tiktok({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      const buf = await fetchVideoBuffer(r.url);
      await sock.sendMessage(ctx.remoteJid, { video: buf, caption: `🎵 ${r.title || 'TikTok'}`, mimetype: 'video/mp4' }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async instagram({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      if (r.type === 'video') {
        const buf = await fetchVideoBuffer(r.url);
        await sock.sendMessage(ctx.remoteJid, { video: buf, caption: '📸 Instagram', mimetype: 'video/mp4' }, { quoted: msg });
      } else {
  ...
  async fb({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      const buf = await fetchVideoBuffer(r.url);
      await sock.sendMessage(ctx.remoteJid, { video: buf, caption: '📘 Facebook', mimetype: 'video/mp4' }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async twitter({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      const buf = await fetchVideoBuffer(r.url);
      await sock.sendMessage(ctx.remoteJid, { video: buf, caption: '🐦 X / Twitter', mimetype: 'video/mp4' }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async soundcloud({ sock, msg, ctx, args }) {
    if (!args.length) return reply(sock, msg, ctx, '☁️ Use: !soundcloud <link ou nome da música>');
    await react(sock, msg, '⏳');
    try {
      const r = await downloader.soundcloud(args.join(' '));
      await sock.sendMessage(ctx.remoteJid, {
        audio: { url: r.url }, mimetype: 'audio/mpeg', fileName: `${r.title || 'SoundCloud'}.mp3`,
      }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
