  ...
  async menubtn({ sock, msg, ctx, config }) {
    ...
    try {
        const buttons = [
          ...
          { id: `${p}menuia`, text: '🧠 IA' },
          { id: `${p}menustickers`, text: '🎨 Stickers' },
...
╰━━━━━━━━━━━━━━━╯`);
  ...
  async pinterest({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await reply(sock, msg, ctx, `📌 Enviando ${results.length} fotos uma por uma...`);
      for (let i = 0; i < results.length; i++) {
        try {
          await sock.sendMessage(ctx.remoteJid, { image: { url: results[i].url }, caption: `📌 ${i+1}/${results.length}` });
