...
async function sendStyledCommandList(sock, msg, ctx, config, { title, subtitle, buttonText = '⚡ Abrir comandos', items = [] }) {
  ...
  try {
    await buttonHandler.sendList(sock, ctx.remoteJid, `⌁ ${config.bot.name} 🌑`, `Selecione um comando de ${title}:`, buttonText, [{ title, rows }], msg);
  } catch (e) {
    try { await buttonHandler.sendButtons(sock, ctx.remoteJid, `⌁ ${title}`, 'Dark Side Engine ⚡', visible.slice(0, 8), msg); } catch {}
  }
}
...
╚══════════════════════╝`;
  ...
  async menubtn({ sock, msg, ctx, config }) {
    ...
    const footer = `⌁ ${config.owner.name} • ${config.bot.name} 🌑`;

    let items = [
      { id: `${p}menudownload`, text: '📥 Downloads', desc: 'play, vídeo, redes sociais' },
      { id: `${p}menustickers`, text: '🎨 Stickers', desc: 'sticker, sfull, attp, ttp' },
      { id: `${p}menujogos`, text: '🎮 Jogos', desc: 'quiz, forca, truco, economia' },
      { id: `${p}menuia`, text: '🧠 IA', desc: 'chat, imagem, APIs' },
      { id: `${p}menugrupo`, text: '👥 ADM/Grupos', desc: 'ban, warn, regras, inatividade' },
      { id: `${p}menu`, text: '📜 Menu Texto', desc: 'lista completa em texto' },
    ];
    if (ctx.isOwner) items.push({ id: `${p}maiscmds`, text: '👑 Dono', desc: 'painel owner/root' });
    ...
    const textFallback = `${title}\n\n` +
      ...
      `\n╰━━━━━━━━━━━━━━━━╯\n\n💡 Se os botões/lista não aparecerem no teu WhatsApp, digita o comando mostrado usando o prefixo configurado.`;

    await reply(sock, msg, ctx, textFallback);

    const rows = items.map(b => ({ title: b.text.slice(0, 24), description: b.desc || b.id, id: b.id }));
    try {
      await buttonHandler.sendList(sock, ctx.remoteJid, `⌁ ${config.bot.name} 🌑`, 'Escolha um módulo do Dark Side Engine:', '⚡ Abrir módulos', [{ title: 'Módulos principais', rows }], msg);
    } catch (e) {
      try { await buttonHandler.sendButtons(sock, ctx.remoteJid, '⚡ Escolha um módulo:', footer, items.map(x => ({ id: x.id, text: x.text })), msg); } catch {}
    }
    logCmd('menubtn', ctx);
  },
...
╰━━━━━━━━━━━━━━━╯`);
  ...
  async aiapis({ sock, msg, ctx }) {
    const status = [
      `Groq: ${process.env.GROQ_API_KEY ? '✅ Render ENV' : '🛑 vazio'}`,
      `Gemini: ${process.env.GEMINI_API_KEY ? '✅ Render ENV' : '🛑 vazio'}`,
      `OpenRouter: ${process.env.OPENROUTER_API_KEY ? '✅ Render ENV' : '🛑 vazio'}`,
      `OpenAI: ${process.env.OPENAI_API_KEY ? '✅ Render ENV' : '🛑 vazio'}`,
      `AI_MODEL: ${process.env.AI_MODEL || 'auto'}`,
    ].join('\n');
    return reply(sock, msg, ctx, `🧠 *APIs DE IA — RENDER*\n\n${status}\n\nConfigure no Render → Environment. Nunca coloque chaves no GitHub.`);
  },
