  ...
  if (mediaUrl && mediaType !== 'none') {
    try {
      ...
      if (mediaType === 'video' || mediaType === 'gif') {
        return sock.sendMessage(ctx.remoteJid, { video: buf, gifPlayback: mediaType === 'gif', caption: finalText }, { quoted: msg });
      } else {
...
/** Baixa vídeo como buffer com limite de tamanho para WhatsApp */
async function fetchVideoBuffer(url, maxMB = 14) {
...
╰━━━━━━━━━━━━━━━╯`);
  ...
  async video({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sock.sendMessage(ctx.remoteJid, { video: await fetchVideoBuffer(r.url), caption: `🎬 *${r.title}*` }, { quoted: msg });
      await react(sock, msg, '✅');
  ...
  async tiktok({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sock.sendMessage(ctx.remoteJid, { video: await fetchVideoBuffer(r.url), caption: `🎵 ${r.title || 'TikTok'}` }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async instagram({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      const payload = r.type === 'video' ? { video: await fetchVideoBuffer(r.url), caption: '📸 Instagram' } : { image: { url: r.url }, caption: '📸 Instagram' };
  ...
  async fb({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sock.sendMessage(ctx.remoteJid, { video: await fetchVideoBuffer(r.url), caption: '📘 Facebook' }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
  ...
  async twitter({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      await sock.sendMessage(ctx.remoteJid, { video: await fetchVideoBuffer(r.url), caption: '🐦 X / Twitter' }, { quoted: msg });
      await react(sock, msg, '✅');
    } catch (e) { await react(sock, msg, '❌'); return reply(sock, msg, ctx, '❌ ' + e.message); }
  },
