...
╚══════════════════════╝`;
  ...
  async menubtn({ sock, msg, ctx, config }) {
    ...
    for (let i = 0; i < 2; i++) {
      try {
        await buttonHandler.sendList(sock, ctx.remoteJid, `⌁ ${config.bot.name} 🌑`, 'Escolha um módulo do Dark Side Engine:', '⚡ Abrir módulos', [{ title: 'Dark Side Modules ⚡', rows }], msg);
        logCmd('menubtn', ctx);
        return;
      } catch {}
      try {
        await buttonHandler.sendButtons(sock, ctx.remoteJid, title, footer, items.map(x => ({ id: x.id, text: x.text })), msg);
        logCmd('menubtn', ctx);
        return;
      } catch {}
    }
