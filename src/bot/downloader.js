...
async function youtubeSearch(query) {
  ...
  if (r.videos?.[0]) return { url: r.videos[0].url, title: r.videos[0].title, id: r.videos[0].videoId };
...
async function youtubeAudio(query) {
  ...
  return tryApis([
    ...
    if (u) return { title: r?.result?.title || search.title, url: u };
    return null;
  });
}
...
async function youtubeAudioSavefrom(query) {
  ...
  try {
    ...
    if (d) {
      ...
      const s = d.stream;
      if (s?.mp3) { const mp3 = Object.values(s.mp3)[0]; if (mp3?.url && !mp3.url.includes('#local')) return { title, url: mp3.url }; }
      for (const q of ['360','240']) { if (s?.mp4?.[q]?.streams?.[0]) return { title, url: `https://worker.sf-tools.com${s.mp4[q].streams[0]}` }; }
    }
  } catch (e) {}
...
async function youtubeAudioAuto(query) {
  ...
  try { const r = await soundcloud(query); r.title += ' (SoundCloud)'; return r; } catch (e) {}
...
async function youtubeVideoSavefrom(query) {
  ...
  throw new Error('❌ SaveFrom não conseguiu baixar vídeo.');
}
      ...
      if (search?.data?.length) {
        ...
        if (scUrl) {
          ...
          try {
            ...
            if (u) return { title: dl?.result?.title || track.permalink || query, url: u };
          } catch (e) {}
        }
      }
    } catch (e) {}
