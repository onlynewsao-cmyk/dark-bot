...
async function youtubeVideoSavefrom(query) {
  ...
  try {
    ...
    const u = r?.result?.download_url || r?.result?.url || r?.url;
    if (u) return { title: search.title, quality: '720p', url: u };
  } catch (e) {}
  return youtubeVideo(query);
}

// !play3 — Auto (davidcyril → ryzendesu → SoundCloud)
...
// !play2 — Fallback DavidCyril alternativo
async function youtubeAudioSavefrom(query) {
  const search = await youtubeSearch(query);
  try {
    const r = await mediaHandler.fetchJson(`https://api.davidcyriltech.my.id/download/ytmp3?url=${encodeURIComponent(search.url)}`);
