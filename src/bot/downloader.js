...
async function downloadAudioFile(query, { bitrate = '128k', label = 'áudio', timeoutMs = 180000 } = {}) {
  ...
  if (bitrate === '320k') {
    try { media = await resolveMedia(query); return await ytdlpAudio(media, bitrate, label, timeoutMs); }
    catch (e) { console.log('[DL-AUDIO] yt-dlp alta falhou, usando entrega rápida:', e.message); }
  }

  // 1ª tentativa: SystemZone direto pelo termo/link. Evita falhar antes em yt-search.
  try {
    ...
    if (z?.url) {
      ...
      if (videoBuffer && videoBuffer.length > 4096) {
        ...
        return {
          title: z.title || media?.title || String(query),
          duration: z.duration || media?.duration || '',
          author: media?.author || '',
          thumb: z.thumbnail || media?.thumb || '',
          url: '',
          buffer,
          mimetype: 'audio/mpeg',
          quality: `${label} · Dark Delivery`,
          fileName: `${safeTitle(z.title || media?.title || String(query))}.mp3`,
        };
      }
    }
  } catch (e) { console.log('[DL-AUDIO] SystemZone falhou:', e.message); }

  media = await resolveMedia(query);
  assertDurationAllowed(media.seconds || 0, media.title);

  // 2ª tentativa: yt-dlp local. Para play3/alta qualidade, também pode ser preferido.
  try {
    media = media || await resolveMedia(query);
    return await ytdlpAudio(media, bitrate, label, timeoutMs);
  } catch (e) { console.log('[DL-AUDIO] yt-dlp falhou:', e.message); }

  throw new Error('❌ Não consegui entregar o áudio agora. A fonte principal falhou e o servidor bloqueou o fallback. Tente outro termo ou vídeo mais curto.');
}
...
async function downloadVideoFile(query, { height = 720, label = '720p', timeoutMs = 260000 } = {}) {
  ...
  if (height >= 1080) {
    try { media = await resolveMedia(query); return await ytdlpVideo(media, height, label, timeoutMs); }
    catch (e) { console.log('[DL-VIDEO] yt-dlp FHD falhou, usando entrega rápida:', e.message); }
  }

  // 1ª tentativa: SystemZone MP4 direto pelo termo/link. Rápido e sem cookie/proxy.
  try {
    const z = await systemZoneYtVideo(query);
    if (z?.url) {
      assertDurationAllowed(parseDurationToSeconds(z.duration), z.title || String(query));
      ...
      if (buffer && buffer.length > 4096) {
        return {
          ...
          buffer,
          mimetype: 'video/mp4',
          quality: z.quality || label,
          fileName: `${safeTitle(z.title || media?.title || String(query))}.mp4`,
        };
      }
    }
  ...
  // 2ª tentativa: yt-dlp local. Para video2/FHD, tenta melhor qualidade real.
  try {
    media = media || await resolveMedia(query);
    return await ytdlpVideo(media, height, label, timeoutMs);
  } catch (e) { console.log('[DL-VIDEO] yt-dlp falhou:', e.message); }

  throw new Error('❌ Não consegui entregar o vídeo agora. A fonte principal falhou e o servidor bloqueou o fallback. Tente outro termo ou vídeo mais curto.');
}
