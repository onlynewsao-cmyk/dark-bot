...
async function downloadAudioFile(query, { bitrate = '128k', label = 'áudio', timeoutMs = 180000 } = {}) {
  ...
  // 1ª tentativa: SystemZone MP4 → MP3 local. Foi a rota mais estável nos testes.
  try {
    const z = await systemZoneYtVideo(media.url || query);
    if (z?.url) {
...
async function downloadVideoFile(query, { height = 720, label = '720p', timeoutMs = 260000 } = {}) {
  ...
  // 1ª tentativa: SystemZone MP4 direto. Rápido e sem cookie/proxy.
  try {
    const z = await systemZoneYtVideo(media.url || query);
    if (z?.url) {
