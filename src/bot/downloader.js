...
async function soundcloud(query) {
  ...
  if (!isUrl) {
    // BUSCA POR NOME — API primeiro (como era antes, mais rápida e confiável)
    try {
      ...
      if (search?.data?.length) {
        ...
        if (scUrl) {
          // 1) Download direto via siputzx (como estava antes)
          try {
          ...
          // Último fallback: retorna link do SoundCloud para usuário baixar
  ...
  // URL direta — API primeiro, yt-dlp fallback
  try {
...
async function pinterestSearch(query, limit = 10) {
  return tryApis([
    ...
    if (Array.isArray(arr) && arr.length) {
      // Filtra imagens reais (prefere pinimg.com mas aceita qualquer HTTP)
      ...
      // Embaralha e pega até `limit` imagens diferentes
