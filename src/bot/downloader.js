...
async function downloadAudioFile(query, { bitrate = '128k', label = 'áudio', timeoutMs = 180000 } = {}) {
  ...
  throw new Error('❌ Não consegui baixar o áudio agora. Ative YTDLP_COOKIES_BASE64 no Render ou configure YTDLP_PROXY se o IP do servidor estiver bloqueado pelo YouTube.');
}
...
async function downloadVideoFile(query, { height = 720, label = '720p', timeoutMs = 260000 } = {}) {
  ...
  throw new Error('❌ Não consegui baixar o vídeo agora. Tente !play para áudio ou configure YTDLP_COOKIES_BASE64/YTDLP_PROXY no Render.');
}
