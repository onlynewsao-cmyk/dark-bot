...
const {
  ...
  externalYoutubeDownload, systemZoneYtVideo, systemZoneSpotifySearch, systemZoneSpotifyDownload, systemZoneSoundCloudSearch, systemZoneSoundCloudDownload, systemZoneTwitter, cobaltDownload, tikwmDownload, spotifydownDownload,
...
async function downloadAudioFile(query, { bitrate = '128k', label = 'áudio', timeoutMs = 180000 } = {}) {
  ...
  // 3ª tentativa: SystemZone MP4 → extrai MP3 com ffmpeg (não depende de cookies/proxy)
  try {
    const z = await systemZoneYtVideo(media.url || media.title || query);
    if (z?.url) {
      const videoBuffer = await fetchMediaBuffer(z.url, 120000);
      if (videoBuffer && videoBuffer.length > 4096) {
        const buffer = extractAudioFromVideoBuffer(videoBuffer, bitrate);
        return {
          title: z.title || media.title, duration: z.duration || media.duration, author: media.author,
          thumb: z.thumbnail || media.thumb, url: '', buffer, mimetype: 'audio/mpeg',
          quality: `${label} · SystemZone MP4→MP3`, fileName: `${safeTitle(z.title || media.title)}.mp3`,
        };
      }
    }
  } catch (e) { console.log('[DL-AUDIO] SystemZone video2audio falhou:', e.message); }

  // 4ª tentativa: loader.to (pode retornar HTML com ads)
  try {
    const quality = bitrate === '320k' ? '320' : bitrate === '192k' ? '192' : bitrate === '160k' ? '192' : '128';
  ...
  // 6ª tentativa: yt-dlp (pode falhar se IP bloqueado)
  try {
