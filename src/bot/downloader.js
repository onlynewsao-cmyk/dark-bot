...
async function youtubeAudio(query) {
  ...
  const apis = [
    // API 1: NexOracle (estável)
    { url: `https://api.nexoracle.com/download/ytmp3?apikey=free_key&url=${encodeURIComponent(search.url)}`, name: 'nexoracle' },
    // API 2: Ryzendesu
    { url: `https://api.ryzendesu.vip/api/downloader/ytmp3?url=${encodeURIComponent(search.url)}`, name: 'ryzendesu' },
    // API 3: DavidCyril
    { url: `https://api.davidcyril.com/api/ytmp3?url=${encodeURIComponent(search.url)}`, name: 'davidcyril' },
    // API 4: Siputzx
    { url: `https://api.siputzx.my.id/api/d/ytmp3?url=${encodeURIComponent(search.url)}`, name: 'siputzx' },
    // API 5: Vreden
    { url: `https://api.vreden.my.id/api/ytmp3?url=${encodeURIComponent(search.url)}`, name: 'vreden' },
  ];
...
async function apkDownload(query) {
  // Método avançado: busca em múltiplas fontes e tenta pegar link direto
  const apis = [
    // API 1: APKMirror via API
    { url: `https://api.ryzendesu.vip/api/search/apk?query=${encodeURIComponent(query)}`, name: 'ryzendesu' },
    // API 2: APKCombo
    { url: `https://api.siputzx.my.id/api/s/apk?query=${encodeURIComponent(query)}`, name: 'siputzx' },
  ];
  ...
  for (const {url,name} of apis) {
    try {
      ...
      if (Array.isArray(arr) && arr.length) {
        const app = arr[0];
        const downloadUrl = app.download_url || app.url || app.link;
        const name = app.name || app.title || query;
        const size = app.size || '';
        const version = app.version || '';
        if (downloadUrl && downloadUrl.startsWith('http')) {
          return { title: name, url: downloadUrl, size, version, source: name };
        }
      }
    } catch(e) {}
  }
  
  // Fallback: busca LiteAPKs e tenta extrair link direto
  try {
    const search = await liteapks(query);
    if (search.length) {
      const first = search[0];
      // Tenta pegar página e extrair link
      const html = require('child_process').execSync(`curl -sL "${first.url}" -H "User-Agent: Mozilla/5.0" --max-time 15`, {timeout:20000}).toString();
      const dlMatch = html.match(/href="(https?:\/\/[^"]+\.apk[^"]*)"/i);
      if (dlMatch) {
        return { title: first.name, url: dlMatch[1], size: '', version: '', source: first.source };
      }
      // Se não achou direto, retorna página
      return { title: first.name, url: first.url, size: '', version: '', source: first.source, isPage: true };
    }
  } catch(e) {}
