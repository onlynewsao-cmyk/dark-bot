...
function videoToWebpSquare(inputBuf, maxSec = 13) {
...
async function imageToWebpFull(buffer) {
  const sharp = require('sharp');
  return sharp(buffer)
    .resize(512, 512, {
      fit: 'contain',
...
function videoToWebpFull(inputBuf, maxSec = 13) {
  ...
  try {
    ...
    execFileSync(FFMPEG_BIN, [
      ...
      '-vcodec', 'libwebp_anim',
      '-lossless', '0', '-quality', '72', '-compression_level', '4',
...
async function createFull(buffer, { botName, ownerName, userName, groupName, isVideo }) {
  const pack = `${botName} • ${ownerName}`;
  const author = `${userName} | ${groupName || 'PV'} • SFULL`;
  const mime = detectMime(buffer);
  const isGif = mime === 'image/gif';
  const isVid = isVideo || mime === 'video/mp4' || mime === 'video/webm' || mime === 'video/avi';
  ...
  try {
    const webp = await imageToWebpFull(buffer);
    const out = await injectMeta(webp, pack, author);
    if (out && out.length > 200) return out;
  } catch (e) {}
