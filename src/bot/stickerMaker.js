...
function videoToWebpSquare(inputBuf, maxSec = 8) {
  ...
  try {
     ...
     *   → scale aumenta ATÉ que o MENOR lado = 512 (mantém proporção)
     *   → crop corta o excesso pelo CENTRO
     *   → resultado: sempre 512x512, sem barras, sem distorção, sem artefactos
     *
     * fps=15          → suficiente para stickers animados
     * format=rgba     → cor correcta para WebP com transparência
     * -vsync vfr      → variable framerate → evita frames duplicados (causa de riscos!)
     * -an             → sem áudio (não necessário em sticker)
     */
    execFileSync('ffmpeg', [
      '-y',
      '-t', String(maxSec),
