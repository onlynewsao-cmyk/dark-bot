  ...
  async start({ mode = 'qr', phoneNumber = null } = {}) {
    ...
    try {
      ...
      this.sock = makeWASocket({
        ...
        connectTimeoutMs:         20000,  // antes 30s → falha mais rápido e tenta de novo
        keepAliveIntervalMs:      25000,  // ping a cada 25s (dentro do limit do Render)
        retryRequestDelayMs:      200,    // antes 250ms → retry mais rápido
        defaultQueryTimeoutMs:    30000,  // antes 60s → respostas mais rápidas
        msgRetryCounterCache:     undefined, // usa cache padrão do Baileys

        // ── Cache de mensagens para velocidade ────────────────
        // O Baileys usa LRU internamente — aqui apenas garantimos que está activado
        getMessage: async (key) => {
          // Tenta buscar do cache do messageListener
          try {
      ...
      this.sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
          ...
          if (code === DisconnectReason.loggedOut || code === 401) {
            ...
            setTimeout(() => { this.starting = false; this.start({ mode: 'qr' }).catch(e => {}); }, 3000);
          }
        }
      });
