...
async function run(name, fn, ms=60000) {
  ...
  try {
    ...
    else { ok = !!r?.url || Buffer.isBuffer(r?.buffer); summary = JSON.stringify({title:r?.title?.slice?.(0,60)||r?.title, quality:r?.quality, duration:r?.duration, mime:r?.mimetype, url: !!r?.url, buffer: Buffer.isBuffer(r?.buffer) ? r.buffer.length : 0, size:r?.size}).slice(0,300); }
