...
(async () => {
  ...
  check('Grupo novo começa em AURA humana (modo base)',
    (await modes.getMode(G1, { isGroup: true })) === modes.MODE_AURA);
  check('v6.93: mas está ACORDADA por defeito', await modes.isAuraAwake(G1, { isGroup: true }));
  check('Segundo grupo também começa em AURA humana',
    (await modes.getMode(G2, { isGroup: true })) === modes.MODE_AURA);
  ...
  // AURA humana é o modo padrão; a invocação continua sendo uma ação
  // explícita separada para recursos proativos/administrativos.
  check('Grupo 2 permanece acordado e isolado do Grupo 1',
    await modes.isAuraAwake(G2, { isGroup: true }),
    'o estado de um grupo não deve dormir outro');
